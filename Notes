...
Modern language have Predictable timing or a way to handle timing. Old languages such as C or Java (these are general purpose programming languages) does not provide time handling, some libraries were developed later on for this purposes. 
...
__Architecture Exploration__
		Defines the architecture that supports the execution of this functionality. (microncontroller architecture, peripherals, where the microncontrollers are gonna be purchased from).
...
_V-model_ allows tracing the errors, at early stage because the test statements and conditions are written at each level of the development.

## Models
_UML_, _SM_ is a software modeling, 

__Why models?__
Sometimes enviroments make a system difficult to test, either due to enviromental conditions or costs, such as:  space or nuclear reactors.

Modeling has two goals, _documenting_ and _simulating_.

_UML_ currently and usually is used as a documenting tool. Documenting and designing allow reusage and improvement  on the code, because a piece of sofware costs. This makes the software development effective, fast and errors avoidant, also currently known as __model-driven architecture__. 
...
## Model-based design
In our case models are built to test or verify properties (by simulation) or checking.
...
The execution plataform. Matches the logical design (HW architecture or OS) into the SW architecture. Using the API of the OS. Time is property that has to match with the hardware. 

Timing properties such as, period and execution time are parameters that are given but the execution platform will dictate the worst case scenario. 

## REQUIREMENT ANALYSIS

Requirements are linked directly with _validation_ because that's what the user wants or expects. 
1. Good requirements avoids inconsistencies and redundancies.
2. Tell what the problem is, not the solution. 
3. What the system is supposed to do.
4. Requirements are (supossely) the starting pointing of the development.
5. even when the project is built by small cycles (meaning prototypes) in each cycle the set of requirements has to be given.
6. They are given and coordinated by/with marketing staff, productt specialists and other analysts.
7. Requirements are natural language, therefore, it's unconsistent, misunderstood.
